...
foreach v of varlist _all {
	if "`v'" == "year" | "`v'" == "plzkgs" | "`v'" == "nome" continue //kein Unterstrich bei plz_kgs sonst funktioniert destring nicht
	cap replace `v' = "" if `v' == "-"
	cap destring `v', replace
}
...
ren _merge mergelbt2025


drop if mergelbt2025 == 2
drop mergelbt2025
...
*Merge Firm Types (Rechtsformen)
import delimited using "${data_dir}\ebdc\V_LINK_05RECHTSFORM.txt", clear
ren meldungs_id idnum

duplicates report idnum
duplicates tag idnum, gen(dup)
list idnum if dup > 0, sepby(idnum)
drop if dup > 0 & !missing(rechtsform)
duplicates drop idnum, force
drop dup
save "${data_dir}\ebdc\V_LINK_05RECHTSFORM.dta", replace


use "${final_data_dir}\IBS_Firm_Level_merged.dta", clear
merge m:1 idnum using "${data_dir}\ebdc\V_LINK_05RECHTSFORM.dta"
